...
class Model:
    ...
    def sample_action(self, s, eps): # policy
...
def play_one(model, eps, gamma): # gamma: discount factor
    ...
    while not done and iters < 10000:
        ...
        # G = reward = gamma*model.sample_action(observation, eps) # uses SARSA cf) doesn't work
        G = reward + gamma*np.max(model.predict(observation)) # since it is q_learning uses np.max **not using policy**
